            ...
            {log.details && (
              <>
                <button
                  ...
                  className="text-xs text-gray-500 dark:text-gray-400 hover:text-gray-950 dark:hover:text-white transition-colors"
                >
...
export function EventLogsTab() {
  ...
  const exportAsPDF = () => {
    try {
      ...
      // Add subtitle with Syntax Stage
      doc.setFontSize(12);
      doc.setFont('helvetica', 'normal');
      doc.text('Syntax Stage - Web Development Playground', margin, 45);
      yPos = 70;
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        // Add footer text
        doc.text('Generated by Syntax Stage', margin, doc.internal.pageSize.getHeight() - 10);
  ...
  const ExportButton = () => {
    ...
    return (
      <DialogRoot open={isOpen} onOpenChange={handleOpenChange}>
        ...
        >
          <span className="i-ph:download text-lg text-gray-500 dark:text-gray-400 group-hover:text-gray-950 dark:group-hover:text-white transition-colors" />
          Export
        </button>
          ...
          <DropdownMenu.Portal>
            ...
            >
              {logLevelOptions.map((option) => (
                <DropdownMenu.Item
                  key={option.value}
                  className="group flex items-center px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-950/10 dark:hover:bg-white/20 cursor-pointer transition-colors"
                  onClick={() => handleLevelFilterChange(option.value)}
                >
                  <div className="mr-3 flex h-5 w-5 items-center justify-center">
                    <div
                      className={classNames(option.icon, 'text-lg group-hover:text-gray-950 dark:group-hover:text-white transition-colors')}
                      style={{ color: option.color }}
                    />
                  </div>
                  <span className="group-hover:text-gray-950 dark:group-hover:text-white transition-colors">{option.label}</span>
                </DropdownMenu.Item>
              ))}
            </DropdownMenu.Content>
          </DropdownMenu.Portal>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <Switch
              ...
              className="data-[state=checked]:bg-gray-950 dark:data-[state=checked]:bg-white"
            />
          ...
          >
            <span className="i-ph:arrows-clockwise text-lg text-gray-500 dark:text-gray-400 group-hover:text-gray-950 dark:group-hover:text-white transition-colors" />
            Refresh
          </button>
        ...
        <div className="relative">
          <input
            ...
            className={classNames(
              ...
              'focus:outline-none focus:ring-2 focus:ring-gray-950/20 dark:focus:ring-white/20 focus:border-gray-950 dark:focus:border-white',
              'transition-all duration-200',
            )}
          />
