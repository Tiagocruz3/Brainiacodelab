...
const NotificationsTab = () => {
  ...
  const getNotificationStyle = (level: string, type?: string) => {
    if (type === 'update') {
      return {
        ...
        color: 'text-black dark:text-gray-500 dark:text-black dark:text-gray-400',
        bg: 'hover:bg-black dark:bg-gray-500/10 dark:hover:bg-black dark:bg-gray-500/20',
      };
    }
        ...
        <div className="flex flex-col gap-2">
          ...
          <button
            ...
            className={classNames(
              ...
              'hover:bg-black dark:bg-gray-500/10 dark:hover:bg-black dark:bg-gray-500/20',
  ...
  const filterOptions: { id: FilterType; label: string; icon: string; color: string }[] = [
    { id: 'all', label: 'All Notifications', icon: 'i-ph:bell', color: '#0ea5e9' },
    { id: 'system', label: 'System', icon: 'i-ph:gear', color: '#6b7280' },
    { id: 'update', label: 'Updates', icon: 'i-ph:arrow-circle-up', color: '#0ea5e9' },
  ...
  return (
    <div className="flex h-full flex-col gap-6">
      <div className="flex items-center justify-between">
          ...
          <DropdownMenu.Portal>
            ...
            >
              {filterOptions.map((option) => (
                <DropdownMenu.Item
                  key={option.id}
                  className="group flex items-center px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-black dark:bg-gray-500/10 dark:hover:bg-black dark:bg-gray-500/20 cursor-pointer transition-colors"
                  onClick={() => handleFilterChange(option.id)}
                >
                  <div className="mr-3 flex h-5 w-5 items-center justify-center">
                    <div
                      className={classNames(option.icon, 'text-lg group-hover:text-black dark:text-gray-500 transition-colors')}
                      style={{ color: option.color }}
                    />
                  </div>
                  <span className="group-hover:text-black dark:text-gray-500 transition-colors">{option.label}</span>
                </DropdownMenu.Item>
              ))}
            </DropdownMenu.Content>
          </DropdownMenu.Portal>
        ...
        >
          <span className="i-ph:trash text-lg text-gray-500 dark:text-gray-400 group-hover:text-black dark:text-gray-500 transition-colors" />
          Clear All
        </button>
      </div>
