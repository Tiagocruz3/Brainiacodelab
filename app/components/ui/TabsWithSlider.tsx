...
export function TabsWithSlider({
  ...
  return (
    <div className={classNames('relative flex gap-2', className)}>
      ...
      {tabs.map((tab, index) => (
        <button
          ...
          className={classNames(
            ...
            tab.id === activeTab
              ? classNames('text-white dark:text-gray-950 shadow-sm shadow-gray-950/20 dark:shadow-white/20', activeTabClassName)
              : classNames(
      ...
      {/* Animated slider */}
      <motion.div
        className={classNames('absolute bottom-0 left-0 h-10 rounded-lg bg-gray-950 dark:bg-white -z-10', sliderClassName)}
        initial={false}
        animate={{
